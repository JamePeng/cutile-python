...
// Pack data type and array rank in a single int64_t so they can be used as a
// single constant for looking up the kernel in a family.
// Layout: [63..32: ndim] [31..0: dtype]
static int64_t pack_array_type(const ArrayType& a) {
    ...
    return static_cast<int64_t>(dtype_u | (ndim_u << 32));
}
...
static inline Status validate_array_index_value(
        int64_t value,
        unsigned index_bitwidth,
        const char* overflow_message) {
    if (index_bitwidth != 64 && (value < INT32_MIN || value > INT32_MAX))
        return raise(PyExc_OverflowError, overflow_message);
    return OK;
}
...
static ArraySpecializationBits compute_array_specialization_bits(
        const Word* array_repr, size_t ndim, unsigned dtype_bitwidth,
...
struct ParsedArrayTypeConstants {
    ArrayType arrty;
    ArraySpecializationBits special_bits;
};

static ParsedArrayTypeConstants parse_array_type_constants(ConstantCursor& cursor) {
    ArrayType arrty = unpack_array_type(cursor.next());
    ArraySpecializationBits special_bits;
    special_bits.u64 = cursor.next();
    return {arrty, special_bits};
}

static Status skip_static_axis_values(
        const Vec<int64_t>& dims,
        ConstantCursor& cursor,
        size_t ndim,
        const char* annotation) {
    for (int64_t dim : dims) {
        Result<size_t> axis = normalize_dim(dim, ndim, annotation);
        if (!axis.is_ok()) return ErrorRaised;
        (void)cursor.next();
    }
    return OK;
}
...
static Result<ArrayRepr> arrayrepr_fake_array(PyObject* pyobj, unsigned, Arena& arena) {
    PyPtr py_dtype = getattr(pyobj, kFakeArrayDTypeAttr);
    ...
    if (!*dtype)
        return raise(PyExc_TypeError, "unsupported fake array dtype ", use_repr(py_dtype.get()));

    PyPtr py_ndim = getattr(pyobj, kFakeArrayNdimAttr);
    if (!py_ndim) return ErrorRaised;
    if (!PyLong_Check(py_ndim.get()))
        return raise(PyExc_TypeError,
                     "fake array _cuda_lang_fake_array_ndim metadata is not an integer");
    Py_ssize_t ndim = PyLong_AsSsize_t(py_ndim.get());
    if (PyErr_Occurred()) return ErrorRaised;
    ASSERT_NDIM(ndim);

    // Native launch-site creation retains only the dtype and rank. Allocate the
    // representation required by the shared extraction path, but leave its
    // pointer, shape, and stride words value-initialized to zero.
...
static Result<ArrayRepr> arrayrepr_dlpack_common(PyObject* dlpack_capsule, unsigned index_bitwidth,
    ...
    for (uint32_t i = 0; i < ndim; ++i) {
        if (!validate_array_index_value(
                    tensor->dl_tensor.shape[i], index_bitwidth, "shape is too big"))
            return ErrorRaised;
    ...
    if (!tensor->dl_tensor.strides) {
        ...
        for (uint32_t i = 0; i < ndim; ++i) {
            if (!validate_array_index_value(
                        tensor->dl_tensor.strides[i], index_bitwidth, "stride is too big"))
                return ErrorRaised;
...
static Result<ArrayRepr> arrayrepr_torch_tensor_pymethod(PyObject* tensor, unsigned index_bitwidth,
    ...
    for (uint32_t i = 0; i < ndim; ++i) {
        ...
        if (!validate_array_index_value(si, index_bitwidth, "shape is too big"))
            return ErrorRaised;
    ...
    for (uint32_t i = 0; i < ndim; ++i) {
        ...
        long long si = PyLong_AsLongLong(item_ptr);
        if (PyErr_Occurred()) return ErrorRaised;
        if (!validate_array_index_value(si, index_bitwidth, "stride is too big"))
            return ErrorRaised;
...
static inline void append_unsigned_int_constant(uint64_t value, Vec<int64_t>* constants) {
    if (value <= static_cast<uint64_t>(INT64_MAX)) {
        append_signed_int_constant(static_cast<int64_t>(value), constants);
    } else {
        constants->push_back(static_cast<int64_t>(PylongConstantEncoding::U64));
        constants->push_back(static_cast<int64_t>(value));
    }
}
...
static PyPtr parse_int_constant_value(ConstantCursor& cursor) {
...
static inline ErrorRaised_t raise_signed_kernel_integer_overflow(unsigned bitwidth) {
    ...
    return raise(PyExc_OverflowError,
                 ...
                 bitwidth == 32 ? "int32_t" : "int64_t");
}

static Status validate_signed_kernel_integer(int64_t value, unsigned bitwidth) {
    CHECK(bitwidth == 32 || bitwidth == 64);
    if (bitwidth == 32 && (value < INT32_MIN || value > INT32_MAX))
        return raise_signed_kernel_integer_overflow(bitwidth);
    return OK;
}

static Status validate_unsigned_as_signed_kernel_integer(uint64_t value, unsigned bitwidth) {
    CHECK(bitwidth == 32 || bitwidth == 64);
    uint64_t max = bitwidth == 32 ? INT32_MAX : INT64_MAX;
    if (value > max)
        return raise_signed_kernel_integer_overflow(bitwidth);
    return OK;
}

static Result<int64_t> pylong_as_signed_kernel_integer(PyObject* pyobj, unsigned bitwidth) {
    CHECK(bitwidth == 32 || bitwidth == 64);
    int overflow = 0;
    long long value = PyLong_AsLongLongAndOverflow(pyobj, &overflow);
    if (PyErr_Occurred()) return ErrorRaised;
    if (overflow) return raise_signed_kernel_integer_overflow(bitwidth);
    if (!validate_signed_kernel_integer(value, bitwidth)) return ErrorRaised;
    return static_cast<int64_t>(value);
}

static inline Status extract_py_long(PyObject* pyobj, unsigned bitwidth, LaunchHelper& helper) {
    Result<int64_t> value = pylong_as_signed_kernel_integer(pyobj, bitwidth);
    if (!value.is_ok()) return ErrorRaised;

    if (bitwidth == 64) {
        push_single_word_cuarg(helper, {.i64 = *value});
    } else {
        CHECK(bitwidth == 32);
        push_single_word_cuarg(helper, {.i32 = static_cast<int32_t>(*value)});
    }
    return OK;
}
...
static PyPtr parse_float_constant_value(ConstantCursor& cursor) {
...
static PyObject* parse_identity_constant_value(ConstantCursor& cursor,
...
static Result<ArrayRepr> get_array_repr(PythonArgKind kind, PyObject* pyobj,
    ...
    switch (kind) {
        ...
        case PythonArgKind::FakeArray:
            return arrayrepr_fake_array(pyobj, index_bitwidth, arena);
        default:
...
static Result<bool> native_integer_is_unsigned(PyObject* fake_scalar) {
    PyPtr py_dtype = steal(
            PyObject_GetAttrString(fake_scalar, kNativeSourceDTypeAttr));
    if (!py_dtype) return ErrorRaised;
    ...
    if (!*dtype)
        return raise(PyExc_TypeError,
                "unsupported native integer launch-source dtype ",
                use_repr(py_dtype.get()));
    DLDataType dt = **dtype;
    if (dt.lanes != 1 || (dt.code != kDLInt && dt.code != kDLUInt)) {
        return raise(PyExc_TypeError,
                "unsupported native integer launch-source dtype ",
                use_repr(py_dtype.get()));
    }
    return dt.code == kDLUInt;
}

static Result<NativeArgument> parse_native_identity_constant_argument(
        ConstantCursor& cursor,
        const Vec<PyObject*>& identity_constants) {
    NativeArgument argument;
    argument.parameter_category = ParameterKind::IdentityConstant;
    argument.identity_constant = newref(parse_identity_constant_value(cursor, identity_constants));
    return argument;
}

static Result<NativeArgument> parse_native_array_argument(
        ConstantCursor& cursor,
        const LeafAnnotationNode& annotation) {
    NativeArgument argument;
    argument.parameter_category = ParameterKind::Array;
    argument.arrty = parse_array_type_constants(cursor).arrty;

    if (!skip_static_axis_values(
                annotation.array.static_shape_dims, cursor,
                argument.arrty.ndim, "static_shape_dims"))
        return ErrorRaised;
    if (!skip_static_axis_values(
...
static Result<NativeArgument> parse_native_scalar_argument(
        ConstantCursor& cursor,
        ParameterKind::Category category,
        PyObject* source_argument) {
    if (!consume_native_scalar_constant(cursor, category))
        return ErrorRaised;

    NativeArgument argument;
    argument.parameter_category = category;
    switch (category) {
    case ParameterKind::ConstantInt:
    case ParameterKind::Integer: {
        Result<bool> host_integer_is_unsigned =
                native_integer_is_unsigned(source_argument);
        if (!host_integer_is_unsigned.is_ok()) return ErrorRaised;
        argument.host_integer_is_unsigned = *host_integer_is_unsigned;
        break;
    }
    default:
        break;
...
static Result<NativeArgument> parse_native_leaf_argument(
        ConstantCursor& cursor,
        ParameterKind::Category category,
        const LeafAnnotationNode& annotation,
        PyObject* source_argument,
    ...
    case ParameterKind::Integer:
    case ParameterKind::Float:
        return parse_native_scalar_argument(cursor, category, source_argument);
    case ParameterKind::AggregateBegin:
    case ParameterKind::AggregateEnd:
        CHECK_UNREACHABLE;
    }
...
static Result<Vec<NativeArgument>> parse_native_arguments(
        ConstantCursor cursor,
        const Vec<PyObject*>& identity_constants,
        const Vec<ParameterKind>& param_kinds,
        const Vec<RefPtr<LeafAnnotationNode>>& flat_param_annotations,
        const Vec<PyObject*>& source_arguments) {
    Vec<NativeArgument> arguments;
    arguments.reserve(flat_param_annotations.size());

    Cursor<RefPtr<LeafAnnotationNode>> annotation_cursor(flat_param_annotations);
    Cursor<PyObject*> source_cursor(source_arguments);
    for (const ParameterKind& param : param_kinds) {
        if (param.category == ParameterKind::AggregateBegin
                || param.category == ParameterKind::AggregateEnd)
            continue;

        Result<NativeArgument> argument = parse_native_leaf_argument(
                cursor, param.category, *annotation_cursor.next(), source_cursor.next(),
                identity_constants);
        if (!argument.is_ok()) return ErrorRaised;
        arguments.push_back(std::move(*argument));
    }
    CHECK(cursor.len == 0);
    CHECK(annotation_cursor.len == 0);
    CHECK(source_cursor.len == 0);
    return arguments;
}
...
Result<NativeLaunchSite*> native_launch_site_create(
    ...
    Result<Vec<NativeArgument>> native_args = parse_native_arguments(
            helper->constants,
            helper->identity_constants,
            profile->family->param_kinds,
            profile->flat_param_annotations,
            helper->leaf_pyarg_objs);
    if (!native_args.is_ok()) return ErrorRaised;

    return new NativeLaunchSite{
        newref(dispatcher_object),
        profile->family,
        profile->flat_param_annotations,
        std::move(*native_args)
    };
}
...
static Status extract_native_scalar(
        const NativeArgument& argument,
        const LeafAnnotationNode& annotation,
        const void* address,
        LaunchHelper& helper) {
    switch (argument.parameter_category) {
    case ParameterKind::ConstantBool: {
        int64_t value = *static_cast<const int64_t*>(address);
        helper.constants.push_back(value != 0);
        return OK;
    }
    case ParameterKind::ConstantInt:
        if (argument.host_integer_is_unsigned) {
            uint64_t value = *static_cast<const uint64_t*>(address);
            append_unsigned_int_constant(value, &helper.constants);
        } else {
            int64_t value = *static_cast<const int64_t*>(address);
            append_signed_int_constant(value, &helper.constants);
        }
        return OK;
    case ParameterKind::ConstantFloat: {
        double value = *static_cast<const double*>(address);
        append_float_constant(value, &helper.constants);
        return OK;
    }
    case ParameterKind::ConstantNone:
        return OK;
    case ParameterKind::Boolean: {
        int64_t value = *static_cast<const int64_t*>(address);
        push_single_word_cuarg(helper, {.i32 = value != 0});
        return OK;
    }
    case ParameterKind::Integer: {
        if (argument.host_integer_is_unsigned) {
            uint64_t value = *static_cast<const uint64_t*>(address);
            if (!validate_unsigned_as_signed_kernel_integer(
                        value, annotation.scalar.bitwidth))
                return ErrorRaised;
            if (annotation.scalar.bitwidth == 64) {
                push_single_word_cuarg(helper, {.i64 = static_cast<int64_t>(value)});
            } else {
                CHECK(annotation.scalar.bitwidth == 32);
                push_single_word_cuarg(helper, {.i32 = static_cast<int32_t>(value)});
            }
            return OK;
        }
        ...
        int64_t value = *static_cast<const int64_t*>(address);
        if (!validate_signed_kernel_integer(
                    value, annotation.scalar.bitwidth))
            return ErrorRaised;
        if (annotation.scalar.bitwidth == 64) {
            push_single_word_cuarg(helper, {.i64 = value});
    ...
    return raise(PyExc_TypeError, "unsupported native launch scalar argument");
}


static Status extract_native_array(
        const DriverApi* driver,
        const NativeArgument& argument,
        const LeafAnnotationNode& annotation,
        ...
        size_t* index,
        LaunchHelper& helper) {
    ArenaOffset repr = arena_alloc_words(helper.arena, 1 + 2 * argument.arrty.ndim);
    helper.arena[repr].device_ptr = *reinterpret_cast<void**>(argument_addresses[(*index)++]);
    for (size_t i = 0; i < 2 * argument.arrty.ndim; ++i) {
        int64_t val = *static_cast<const int64_t*>(argument_addresses[(*index)++]);
        if (!validate_array_index_value(
                    val,
                    annotation.array.index_bitwidth,
                    i < argument.arrty.ndim ? "shape is too big" : "stride is too big"))
            return ErrorRaised;
        helper.arena[repr + 1 + i].i64 = val;
    }
    ArrayRepr array = {argument.arrty, repr};
...
static Status extract_native_launch_arguments(
    ...
    size_t index = 0;
    CHECK(site.arguments.size() == site.flat_param_annotations.size());
    for (size_t i = 0; i < site.arguments.size(); ++i) {
        const NativeArgument& argument = site.arguments[i];
        const LeafAnnotationNode& annotation = *site.flat_param_annotations[i];
        switch (argument.parameter_category) {
        case ParameterKind::IdentityConstant:
            extract_identity_constant(
                    argument.identity_constant.get(),
        ...
        case ParameterKind::Integer:
        case ParameterKind::Float:
            if (!extract_native_scalar(
                        argument, annotation,
                        argument_addresses[index++], helper))
                return ErrorRaised;
            break;
        case ParameterKind::ConstantNone:
            break;
...
static Status validate_native_grid(const int64_t dims[3]) {
    Grid grid;
    for (int i = 0; i < Grid::Len; ++i) {
        if (dims[i] < 0)
            return raise(PyExc_ValueError, "Grid[", i,
                         "] value must be non-negative: got=", dims[i]);
        if (dims[i] > UINT_MAX)
            return raise(PyExc_ValueError, "Grid[", i,
                         "] value too big: got=", dims[i]);
        grid.dims[i] = static_cast<unsigned>(dims[i]);
    }
    return validate_grid(grid);
}


static Status validate_native_launch_config(const NativeLaunchConfig& config) {
    const int64_t* dimension_sets[] = {
        config.grid,
        config.block,
        config.has_cluster ? config.cluster : nullptr,
    };
    for (const int64_t* dims : dimension_sets) {
        if (dims && !validate_native_grid(dims))
            return ErrorRaised;
    }

    if (config.has_preferred_cluster) {
        if (!config.has_cluster) {
            return raise(
                    PyExc_ValueError,
                    "Keyword argument preferred_block_in_cluster_count "
                    "requires that block_in_cluster_count is also passed");
        }
        if (!validate_native_grid(config.preferred_cluster))
            return ErrorRaised;
    }
...
int32_t native_launch_site_launch(
    ...
    helper->cuda_context = *stream_context;
    if (!extract_native_launch_arguments(
                driver, *site, argument_addresses, *helper))
        return -1;

    CudaContextGuard context_guard(driver);
    StreamBufferTransaction transaction;
    Result<PreparedLaunch> prepared = prepare_launch_with_extracted_args(
    ...
    if (config.has_cluster) {
        CUlaunchAttribute& attribute = attributes[num_attributes++];
        attribute.id = CU_LAUNCH_ATTRIBUTE_CLUSTER_DIMENSION;
        attribute.value.clusterDim = {
            static_cast<unsigned>(config.cluster[0]),
            static_cast<unsigned>(config.cluster[1]),
            static_cast<unsigned>(config.cluster[2])};
    }
    if (config.has_preferred_cluster) {
        CUlaunchAttribute& attribute = attributes[num_attributes++];
        attribute.id = CU_LAUNCH_ATTRIBUTE_PREFERRED_CLUSTER_DIMENSION;
        attribute.value.preferredClusterDim = {
            static_cast<unsigned>(config.preferred_cluster[0]),
            static_cast<unsigned>(config.preferred_cluster[1]),
            static_cast<unsigned>(config.preferred_cluster[2])};
    }
    ...
    if (config.programmatic_dependent_launch) {
        CUlaunchAttribute& attribute = attributes[num_attributes++];
    ...
    CUlaunchConfig launch_config = {
        .gridDimX = static_cast<unsigned>(config.grid[0]),
        .gridDimY = static_cast<unsigned>(config.grid[1]),
        .gridDimZ = static_cast<unsigned>(config.grid[2]),
        .blockDimX = static_cast<unsigned>(config.block[0]),
        .blockDimY = static_cast<unsigned>(config.block[1]),
        .blockDimZ = static_cast<unsigned>(config.block[2]),
        .sharedMemBytes = prepared->dynamic_smem_bytes,
        .hStream = stream,
        .attrs = num_attributes ? attributes : nullptr,
        .numAttrs = num_attributes,
    };
    CUresult result = driver->cuLaunchKernelEx(
            &launch_config,
            reinterpret_cast<CUfunction>(prepared->kernel),
            make_launch_params(*prepared->helper), nullptr);
