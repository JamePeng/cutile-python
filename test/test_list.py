...
def test_add_list_of_arrays(kernel):
    arrays = [torch.randint(0, 100, (16, 16), dtype=torch.int32, device="cuda:0") for _ in range(3)]
...
def test_add_list_static_shape():
    ...
    with patch('cuda.tile._compile.compile_tile',
        ...
        for shape in shapes:
            arrays = [torch.randint(0, 100, shape, dtype=torch.int32, device="cuda:0")
                      for _ in range(3)]
            out = torch.zeros((16, 16), dtype=torch.int32, device="cuda:0")
...
def test_add_list_static_shape_mismatch():
    ...
    arrays = [
        torch.zeros((16, 16), dtype=torch.int32, device="cuda:0"),
        torch.zeros((32, 16), dtype=torch.int32, device="cuda:0"),
    ]
    out = torch.zeros((16, 16), dtype=torch.int32, device="cuda:0")
...
def test_add_list_of_int64_index_arrays():
    ...
    arrays = [torch.full((n, 1), i + 1, device='cuda:0', dtype=torch.int8) for i in range(3)]
    out = torch.zeros(n, 1, device='cuda:0', dtype=torch.int8)
