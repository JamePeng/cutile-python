...
@ct.kernel
def mma_builtin_function_as_accumulator(A, B, C,
                                        tm: ct.Constant[int],
                                        tn: ct.Constant[int],
                                        tk: ct.Constant[int]):
    bidx = ct.bid(0)
...
@ct.kernel
def mma_uninitialized_var_with_builtin_name(A, B, C,
                                            tm: ct.Constant[int],
                                            tn: ct.Constant[int],
                                            tk: ct.Constant[int]):
    bidx = ct.bid(0)
    ...
    b = ct.load(B, index=(0, bidy), shape=(tk, tn))
    sum2 = ct.mma(a, b, sum) # noqa
    sum = ct.full((tm, tn), 0, dtype=C.dtype) # noqa
    sum2 = ct.astype(sum2, C.dtype)
    ct.store(C, index=(bidx, bidy), tile=sum2)
...
@pytest.mark.parametrize("func", [
    ...
    mma_uninitialized_var_with_builtin_name,
...
def test_builtin_function_as_accumulator():
    with pytest.raises(TileTypeError,
                       match="given value has type <built-in function sum>"):
        _launch(mma_builtin_function_as_accumulator)
