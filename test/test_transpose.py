...
def test_permute_2d(kernel, dtype, use_method):
    x = make_tensor((128, 64), dtype=dtype, device='cuda:0')
    y = torch.zeros((64, 128), dtype=dtype, device='cuda:0')
...
def test_transpose_3d(dtype):
    x = make_tensor((1, 8, 2), dtype=dtype, device='cuda:0')
    y = torch.zeros((1, 2, 8), dtype=dtype, device='cuda:0')
...
def test_transpose_1d():
    ...
    y = torch.zeros((128, ), dtype=torch.float32, device='cuda:0')
...
def test_transpose_2d_single_axis():
    x = make_tensor((128, 64), dtype=torch.float32, device='cuda:0')
    y = torch.zeros((128, 64), dtype=torch.float32, device='cuda:0')
    with pytest.raises(TileTypeError,
...
def test_transpose_3d_default_axes():
    x = make_tensor((128, 64, 32), dtype=torch.float32, device='cuda:0')
    y = torch.zeros((128, 64, 32), dtype=torch.float32, device='cuda:0')
    with pytest.raises(TileTypeError,
